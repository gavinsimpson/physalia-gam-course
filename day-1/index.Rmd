...
date: "1400&ndash;2000 CET (1300&ndash;1900 UTC) Monday 14th, 2022"
output:
...
The Poisson gives the distribution of the number of “things” (individuals, events, counts) in a given sampling interval/effort if each event is \alert{independent}.
...
2. A \alert{Linear Predictor} $\eta$ &mdash; the linear function of regressors
...
- \alert{Poisson} &mdash; counts; integers, non-negative, variance increases with mean
- \alert{Binomial} &mdash; observed proportions from a total; integers, non-negative, bounded at 0 and 1, variance largest at $\pi = 0.5$
- \alert{Binomial} &mdash; presence absence data; discrete values, 0 and 1, models probability of success
- \alert{Gamma} &mdash; concentrations; non-negative (strictly positive with log link) real values, variance increases with mean, many zero values and some high values
...
```{r predict-darlingtonia, echo = TRUE}
...
* Deviance is defined relative to an arbitrary constant &mdash; only \alert{differences} of deviances have any meaning
...
```{r plot-maddy-fitted, echo=FALSE}
...
Call $\boldsymbol{\eta}$ the **linear predictor**
