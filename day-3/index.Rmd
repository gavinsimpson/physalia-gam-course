...
date: "1400&ndash;2000 CET (1300&ndash;1900 UTC) Wednesday 10th December, 2025"
output:
...
* Testing for non-linearity beyond a linear effect
...
Use the `derivatives()` function in *gratia* 📦 provides
...
Estimation of first or second derivatives of smooths (on link scale) using **finite differences**
...
```{r finite-diff-img, echo = FALSE}
knitr::include_graphics("./resources/finite_difference_method.svg")
```
.smaller[
Kakitc, [CC BY-SA 4.0](https://creativecommons.org/licenses/by-sa/4.0), via Wikimedia Commons
]
]
]
...
# Partial effect alternative

Partial effect plots are the default way of visualising component smooths of a GAM

They show how the response varies (on the link scale!) over the range of $\mathbf{x}$ where the effect of all other terms is equal to 0

Inconsistent with the interpretation of $\beta_j$ in a GLM &mdash; the $\beta_j$ are (partial) derivatives, showing change in $y_i$ for a unit (= 1) change in $x_i$

Instead of partial effect plots, use derivatives

---

# Partial effect alternative

Example multi-term GAM with non-identity link function

```{r}
d <- data_sim(seed = 2025-12-10, dist = "tweedie")
m <- gam(y ~ s(x0) + s(x1) + s(x2) + s(x3), data = d, family = tw(), method = "REML")
```
...
```{r}
m |> derivatives(eps = 0.01) |> # 1% 𝚫 in x
  draw()
```
]
...
*gratia* 📦 includes additional functions for computing derivatives

* `partial_derivatives()` will compute the derivative of $x$ with respect to $z$ &mdash; useful for tensor product interactions

* `response_derivatives()` is like `fitted_values()` but for derivatives &mdash; it computes derivatives of the fitted values ($\mathbb{E}(y_i)$) of the model on the response or link scales

    * similar to `marginaleffects::slopes()`

    * uncertainty estimation is via posterior simulation (tomorrow)

* `derivative_samples()` takes draws from the posterior distribution of the derivative of fitted values ($\mathbb{E}(y_i)$)
...
```{r draw-tprs-vs-tensor-product, message = FALSE, out.width = "70%", fig.height = 8, fig.align = "center"}
truth_plt + (draw(m_tprs) + coord_cartesian()) + draw(m_te) + plot_layout(ncol = 2, nrow = 2)
```
...
Tensor product of $x$ and $z$ we start with the marginal bases for both covariates

$f_x(x) = \sum_{i=1}^{I} \alpha_i a_i(x), \; \; f_z(z) = \sum_{l=1}^{L} \delta_l d_l(z)$

For $f_x$ to vary with $z$ we need its parameters $\alpha_i$ to vary with $z$

$\alpha_i(z) = \sum_{l=1}^{L} \delta_l d_l(z)$

leading to

$f_{xz}(x, z) = \sum_{i=1}^{I} \sum_{l=1}^{L} \delta_l d_l(z) a_i(x)$
...
Multiple options for factor smooth interactions

1. factor `by` terms

    * `by` is a factor, or

    * `by` is an *ordered* factor

2. random factor smooths (`bs = "fs"`)

3. constrained factor smooths (`bs = "sz"`)
...
1. a continuous vector &mdash; varying coefficient model (skipped now)
...
1. a factor vector &mdash; a factor-smooth interaction, or
...
Each of the smooths is still centred about 0 so doesn't include the group means

Must include the parametric factor effect of `f` to model the group means
...
2. difference smooths for each other level of the factor (groups), where difference is with respect to the reference group's smooth

```{r, eval = FALSE}
data$o <- ordered(data$f) # convert f into an ordered factor
contrasts(data$o) <- "contr.treatment" # reset contrasts to usual
gam(y ~ o + s(x) + s(x, by = o), data = data, ....)
```

Useful if you have a true reference category

Can be sensitive to how well the reference category is modelled

---

# Constrained factor interactions

This is a newer option, provided by `bs = "sz"`

1. an average or global smooth of $x$, plus

2. a difference smooth for each level of factor `f`

Includes group means, so no need to add parametric effects

```{r, eval = FALSE}
gam(y ~ s(x) + s(x, f, bs = "sz"), data = data, ....)
```

Advantage is that the *average* smooth is, by design, orthogonal to the difference smooths
...
```{r chick-factor-by, fig.align = "center", out.width = "75%"}
...
```{r chick-factor-by-grouped, fig.align = "center", out.width = "75%"}
m_cw_f <- gam(weight ~ diet + s(time, by = diet),
  data = cw, family = tw(), method = "REML")
draw(m_cw_f, grouped_by = TRUE)
```
---

# Ordered factor `by`

```{r chick-ordered-factor-by, fig.align = "center", out.width = "75%"}
...
```{r chick-factor-sz, fig.align = "center", out.width = "75%"}
...
1. ordered factor smooths, and
2. constrained factor smooths

a complication is that the way $\mathbf{y}$ varies with $\mathbf{x}$ for each level of $f$ is not immediately clear

Use `conditional_values()` or `marginaleffects::plot_predictions()`
...
```{r conditional-values-cw-sz}
m_cw_z |> conditional_values(
...
```{r plot-predictions-cw-sz}
m_cw_z |> plot_predictions(
  condition = c("time", "diet")
)
```
]
...
.small[
Note: No difference with this example; in models with more terms, we (auto) condition on slightly different values of the other covariates/terms
]
...
The `fs` basis produces random factor smooths

Smooth equivalent of random intercept *plus* slopes

Most useful for *subject-specific* effects

The set of smooths share a smoothing parameter for wiggliness

Includes subject means and random slopes

---

# Random factor smooths

```{r cw-fs-plot, fig.align = "center", out.width = "65%"}
m_cw_fs <- gam(
  weight ~
...
```{r gam_check_norm4, echo = FALSE, fig.height = 12, out.width = "50%", fig.align = "center"}
...
Model the deviance residuals with same model structure but with increased `k` and special `family`

```{r alt-basis-dim-check-2}
res <- resid(norm_model_1, type = "deviance")

res_model <- gam(
  res ~ s(x1, k = 12) + s(x2, k = 12), # <-- increased k
  method = "REML",
...
.small[
  Note the residuals contain unmodelled wiggliness
]
...
Smooths show the wiggliness that was unmodlled in the *original* fit

```{r alt-basis-dim-check-3, fig.align = "center", out.width = "85%"}
draw(res_model)
```
...
`method = "simulate"` simulates new data from the fitted model

Used on QQ plot to generate the reference band &mdash; if your model can reproduce the observed data, then the residuals should mostly lie within the reference band

Band is by default a 90% reference band &mdash; we expect 10% of residuals to lie outside the band

.smaller[
  N.H. Augustin, E-A Sauleaub, S.N. Wood (2012) On quantile quantile plots for generalized linear models *Computational Statistics & Data Analysis*. **56**(8), 2404&ndash;2409.
]

---

# PIT & quantile residuals

Even when we have the right model, residuals patterns can persist

How do we tell the difference between good and bad patterns?

Randomised quantile and probability integral transform residuals can help

PIT residuals &mdash; should be distributed uniform if model is correct

Quantile residuals &mdash; should be distributed Gaussian if model is correct

---

# PIT & quantile residuals

NB data, Poisson model &mdash; wrong model!

.row[

.col-6[

```{r pit-gam-check-example-pois, fig.height = 5.5}
...
```{r quantile-gam-check-example-pois, fig.height = 5.5}
residuals_linpred_plot(
  pois_model, type = "quantile"
)
```
]
...
```{r pit-gam-check-example-hist-pois, fig.height = 5.5}
residuals_hist_plot(
  pois_model, type = "pit"
)
```
...
```{r quantile-gam-check-example-hist-pois, fig.height = 5.5}
residuals_hist_plot(
  pois_model, type = "quantile"
)
```
...
```{r pit-gam-check-example, fig.height = 5.5}
residuals_linpred_plot(
  negbin_model, type = "pit"
)
```
]
...
```{r quantile-gam-check-example, fig.height = 5.5}
residuals_linpred_plot(
  negbin_model, type = "quantile"
)
```
]
...
NB data, NB model &mdash; correct model!

.row[

.col-6[

```{r pit-gam-check-example-hist, fig.height = 5.5}
residuals_hist_plot(
  negbin_model, type = "pit"
)
```
...
```{r quantile-gam-check-example-hist, fig.height = 5.5}
residuals_hist_plot(
  negbin_model, type = "quantile"
)
```
...
```{r dharma-larks-sim-residuals, fig.align = "center", out.width = "75%"}
